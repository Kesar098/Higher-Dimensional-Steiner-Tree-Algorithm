function SOR(w::Float64, epi::Float64, K::Int, data::Variables, OA::optiArrays)
    V = data.Vertices 
    adj = data.adj 
    EL = data.EL
    ...
    # Relaxation 
    for i in 1:K 
        V[N+i] = w.*(V[N+i]) .+ (1.0-w).*oldV[i]
    end

    return nothing
end

function SOR(scale::Float64, K::Int, data::Variables, OA::optiArrays)
    n=K+2
    w = 0.0
    if n<=5
        w = 1.6
